...
class Vent():
    ...
    def coords(self):
        if self.diag:
            return zip(np.linspace(self.x1, self.x2, abs(self.x1-self.x2)+1), np.linspace(self.y1, self.y2, abs(self.y1-self.y2)+1))
...
for vent in vents:
    for x, y in vent.coords():
        ...
        exposed_vents[coordinate] += 1
        if exposed_vents[coordinate] == 2:
            count += 1
# print(exposed_vents)
# print(vents)
print(count)
